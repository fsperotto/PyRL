...
from .debugger import Debugger, Commands
...
class Sim():
    ...
    def __init__(self, agents, envs, episode_horizon=100, num_episodes=1, num_simulations=1,
                 episode_finished_callback=None, simulation_finished_callback=None, round_finished_callback=None, debug=False):
    ...
    def run(self, episode_horizon=None, num_episodes=None, num_simulations=None):
        ...
        if self.debug:
            ...
            self.debugger.connect(parent_conn)
            env_process = Process(target=self.sim_loop, args=(child_conn, episode_horizon, num_episodes, num_simulations))
            env_process.start()
            self.debugger.run()
            env_process.terminate()
            env_process.join()
        else:
            self.sim_loop(episode_horizon=episode_horizon, num_episodes=num_episodes, num_simulations=num_simulations)

    def sim_loop(self, connection=None, episode_horizon=None, num_episodes=None, num_simulations=None) -> None:
                ...
                for i in range(num_simulations):
                    ...
                    for j in range(num_episodes):
                        ...
                        for t in range(1, episode_horizon+1):
                            if self.debug:
                                continue_loop = True
                                while pause or continue_loop:
                                    if connection.poll():
                                        command = connection.recv()
                                        if command == Commands.QUIT:
                                            return
                                        elif command == Commands.GO:
                                            pause = False
